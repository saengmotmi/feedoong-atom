...
## 3.1 RSS 도메인 핵심 성공요인 (v0.3)

RSS 피드앱에서 사용자 체감 품질을 좌우하는 핵심은 기능 개수보다 아래 2가지다.

1. 파싱 내구성: 불규칙한 RSS/Atom/JSON Feed/HTML 페이지에서도 피드 발견과 정규화가 안정적으로 동작해야 한다.
2. 싱크 전략: 소스별로 "언제 다시 가져올지"를 동적으로 결정해 신선도/비용/차단 리스크를 균형 있게 관리해야 한다.

도메인 관점에서 본 제품 품질 판단은 아래 식으로 본다.

- Feed Quality = f(Discovery Success, Parse Robustness, Sync Freshness, Sync Cost, Failure Recovery)
...
추가 관측(v0.3):
1. `sync-all`은 전체 소스를 `Promise.all` 병렬로 처리한다. (동시성 상한 없음)
2. scheduler는 고정 cron 주기 기반이며, 소스별 next check/backoff 정책은 아직 없다.
3. HEAD preflight 기반 skip 판단은 존재한다.
...
v0.3 도메인 강화 포함:
7. 파싱 실패 원인 분류 표준화(`DISCOVERY`/`FETCH`/`PARSE`/`STORE`)
8. 싱크 정책의 소스 단위 상태 모델 설계(`nextCheckAt`, `errorCount`, `lastErrorAt`, `retryAfter`)
...
v0.3 제외(명시):
5. 멀티 서비스 플러그인 마켓 수준 확장성
6. 완전한 Google Reader/Fever 쓰기 호환(읽기 중심 최소 호환만 목표)
...
5. 파싱 단계에서 discovery 후보/시도 횟수/실패 원인을 구조화해 반환해야 한다.
6. 싱크는 소스별 `nextCheckAt`을 기준으로 수행 대상을 선택할 수 있어야 한다.
7. 429/503 + `Retry-After` 응답 시 소스 단위로 재시도 시점을 지연해야 한다.
8. 동기화 동시성은 전역 상한과 호스트별 상한을 모두 지원해야 한다.
9. 조건부 요청(If-None-Match, If-Modified-Since)을 GET/HEAD 모두에서 활용해야 한다.
...
5. 파서 회귀 테스트 fixture(포맷/도메인별)를 유지해야 한다.
6. 동기화 실패는 소스 단위 부분 실패를 허용하고 전체 배치를 중단하지 않아야 한다.
7. 운영 로그는 `sourceHost`, `strategy`, `candidateUrl`, `statusCode`, `durationMs`를 포함해야 한다.
...
v0.3 도메인 지표:
5. discovery 성공률: 사용자 입력 URL 기준 90%+
6. parse 실패 재발률: 동일 도메인 기준 7일 이동평균 20%↓
7. 429 재발률: 동기화 요청 1,000건당 50%↓ (정책 적용 전 대비)
8. 불필요 fetch 비율: 304/unchanged 판정으로 본문 재다운로드 회피율 60%+
...
3. 소스 단위 next-check + backoff + retry-after 도입
4. 동기화 동시성 상한(전역/호스트별) 도입

### Phase 3

1. 읽음/즐겨찾기
2. 백업/내보내기
3. 파서 fixture 회귀 테스트 확장
4. Google Reader/Fever 읽기 호환 최소셋 검토
...
4. 파서 다중 엔진(fallback parser) 도입 시 복잡도/효익 균형
5. 도메인별 robots/politeness 정책(재시도 간격, UA, host cooldown) 기본값
6. 읽기 상태 동기화(GReader/Fever) 범위를 MVP 이후 어느 시점에 포함할지

## 14. 벤치마크 기반 도메인 해자 분석 (v0.3)
...
1. Miniflux: 조건부 요청 + `next_check_at` 중심 스케줄링 + TTL/Cache-Control/Retry-After 반영
2. CommaFeed: 큐 기반 refresh 엔진 + 경험적 interval + 에러 백오프 + 필터 표현식
3. NetNewsWire: 파서/피드발견 품질 + 대형 fixture 회귀 테스트 자산
4. FreshRSS: GReader/Fever 호환 + 확장 생태계 + 운영 CLI
5. RSS Guard: 플러그인 기반 다중 서비스 연동 + 양방향 동기화

### 14.2 Feedoong 현재 강점과 격차

강점:
1. 국내 도메인 특화 discovery 전략
2. 단순한 운영 구조(1인 유지 관점)
3. HEAD preflight 기반 기본 skip 판단

격차:
1. 파싱 회귀 테스트 자산 부족
2. 소스 단위 동적 싱크 정책(next-check/backoff/retry-after) 부재
3. 읽기 상태/외부 클라이언트 호환 생태계 부재

### 14.3 Feedoong 도메인 포지셔닝 목표

1. 단기(MVP+): "개인 인사이트 수집기" 품질 극대화 (파싱/싱크 내구성 우선)
2. 중기: "호환 가능한 RSS 리더"로 확장 (읽기 중심 API 호환)
3. 장기: 도메인 특화 discovery + 저비용 운영 안정성을 결합한 개인 운영형 해자 확보
